...
class _DropsSnakeState extends State<DropsSnake> {
  ...
  void randomizeLetters() {
    ...
    for (var c = 1; c < currentWord.name.length - 1; c++) {
      wordTree.add(
          [[0,0]]); //ilk katmana kelimedeki harf sayısı kadar liste ekliyoruz.
    }

    for (var j = 1; j < currentWord.name.length; j++) {
      // j kelimedeki current index
      if (currentIndex < row) {
      ...
      for (int l = 0; l < available.length-1; l++) {
        //kutular boş mu diye kontrol ediyor
        if (randomizedLetters[currentIndex + available[l]] == " ") {
          wordTree[j].add([0,0]);
          wordTree[j][l][0]= currentIndex+available[l];
          wordTree[j][l][1]=checkNeigbourCells(currentIndex+available[l]);
        }
      }
  ...
  int checkNeigbourCells (gridIndex){
    List<int> defAvailable;

          if (gridIndex < row) {
        if ((gridIndex + 1) % column == 0) {
          defAvailable = [-1, row]; //top right
        } else if (gridIndex % column == 0) {
          defAvailable = [1, row]; //top left
        } else {
          defAvailable = [-1, 1, row]; //only top
        }
      } else if (gridIndex >= row * column - 1) {
        if ((gridIndex + 1) % column == 0) {
          defAvailable = [-1, -row]; //bottom right
        } else if (gridIndex % column == 0) {
          defAvailable = [1, -row]; //bottom left
        } else {
          defAvailable = [-1, 1, -row]; //only bottom
        }
      } else if ((gridIndex + 1) % column == 0) {
        defAvailable = [-1, -row, row]; //only right
      } else if (gridIndex % column == 0) {
        defAvailable = [1, -row, row]; //only left
      } else {
        defAvailable = [-1, 1, -row, row]; //not edge
      }

      for (int l = 0; l < defAvailable.length-1; l++) {
        //kutular boş mu diye kontrol ediyor
        if (randomizedLetters[gridIndex + defAvailable[l]] != " ") {
          defAvailable.removeAt(l);
        }
      }

    return defAvailable.length;
  }
